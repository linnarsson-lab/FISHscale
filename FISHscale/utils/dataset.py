    ...
    def segment(self,
        ...
        for nx in trange(self.dask_attrs[label_column].npartitions - 1):
            ...
            if labels.max() >= 0:
                ...
                result_grp = Parallel(
                    n_jobs=multiprocessing.cpu_count(), temp_folder='/wsfish/tmp/', mmap_mode='w+', backend='threading')(delayed(_cell_extract)(part.to_dict('list'), self.unique_genes) for _, part in partition_filt.groupby('Segmentation'))
