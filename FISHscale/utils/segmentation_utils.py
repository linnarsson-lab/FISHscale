...
def _segmentation_dots(partition, func, resegmentation_function):
    ...
    resegmentation_data = []
    results_resegmentation = Parallel(n_jobs=multiprocessing.cpu_count(), temp_folder='/wsfish/tmp/',mmap_mode='w+',prefer='threads')(delayed(resegmentation_function)(part) for _, part in partition.groupby('tmp_sement'))
    resegmentation = []
