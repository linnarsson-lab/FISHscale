...
def clust_hex(hex_bin, manifold, n_clusters=None, data=None, clustering=None, labels_input=None, names=[],
...
def clust_hex_connected(hex_bin, data, samples, distance_threshold=None, 
                        n_clusters=None, neighbor_rings=1, names=[], x_lim=None, 
    ...
    labels_all = []
    for i, d in enumerate(list(hex_bin.keys())):
        print(f'Processing sample: {i}', end='\r')
        ...
        #ax.invert_yaxis()
        ax.axis('off')
        ax.set_title(f'{names[i]} {n_clust} clusters')
...
def make_cluster_mean(hex_bin_signal, samples, labels, names):
    ...
    `labels_all`(np.array): Array with the clusteringlabels for each dataset 
...
def hex_neighbour_coord(x,y,s):
    """
    Calculate the cartesian coordinates of the neibours of a point in a hexagonal grid.
...
def hex_region_boundaries(hex_bin, labels):
...
def hex_region_boundaries(hex_bin, labels, decimals=7):
    ...
    for d, label in zip(datasets, labels):
        ...
        for c in hex_bin[d]['hexagon_shape'][0].vertices[:6]: #There are points in the polygon for the closing vertice.
            angle = round(hex_clust.get_rotation(0,0, c[0], c[1]))
            corner[angle] = c
        ...
        for i, l in enumerate(label):
            ...
            if not np.all(neighbour_identity == l) or len(indices) < 6:
                ...
                angles = np.array([round(hex_clust.get_rotation(own_coord[0], own_coord[1], c[0], c[1])) for c in neighbour_coords])
